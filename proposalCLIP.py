...
def clip_objectness(image, init_boxes, init_scores,
					...
					lambda_sim=0.5, lambda_sl=0.2, device="cuda"):
	"""
	2.计算基于CLIP的目标得分

	CLIP提取特征为V_M，与每一类文本特征T_c计算相似度，跟一个softmax
	计算CLIP相似熵E_m，剔除40%高熵值提案，剩下的记为O_t
	计算基于CLIP的目标得分S_t
	...
	# 计算图像区域和文本之间的相似度矩阵 Sim_{m,c}
	sim_matrix = img_feats @ txt_feats.T
	sim_softmax = F.softmax(sim_matrix, dim=1)

	# 计算每个提案的相似度熵 E_m
	E_m = -torch.sum(sim_softmax * torch.log(sim_softmax + 1e-6), dim=1)

	# 根据熵值筛除高熵提案
	N = sim_matrix.shape[0]
	T = int(N * (1 - high_entropy_ratio))
	sorted_entropy, indices = torch.sort(E_m)
	keep_idx = indices[:T]
	...
	return E_m, S_t, keep_idx, img_feats
...
# 主流程入口函数
def main(img, clip_model, preprocess, tokens, device, show=False):
	"""
	主流程逻辑：
	...
	init_boxes, init_scores = edgebox("./EdgeBoxes/model/model.yml.gz", img, 300, show)
...
if __name__ == "__main__":
	...
	classes = [
		"cavity", "crack", "drop", "hole", "leak",
	]

	clip_model, preprocess, tokens = init_CLIP("ViT-B/32", classes, device=device)
	img = cv2.imread("examples/2_drop/0.jpg")

	show = True

	boxes, scores = main(img, clip_model, preprocess, tokens, device, show)

	if show:
		for x, y, w, h in boxes:
			cv2.rectangle(img, (x, y), (x + w, y + h), tuple(np.random.randint(0, 255, 3).tolist()), 2)
